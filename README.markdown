...
insert information into the [list of checks](https://github.com/okeuday/pest/blob/master/pest.erl#L148-L267).
    ...
    [{90,"Port Drivers may cause undefined behavior",
     ...
     {80,"OS shell usage may require input validation",
      [{cpu_sup,get_uint32_measurement,2},
       {ct_webtool,script_start,1},
       {dialyzer_analysis_callgraph,dump_callgraph,4},
       {dialyzer_callgraph,to_ps,3},
       {icpreproc,preproc,2},
       {icscan,call_preproc,2},
       {memsup,freebsd_sysctl,1},
       {memsup,get_memory_usage,1},
       {memsup,get_os_wordsize,1},
       {memsup,get_os_wordsize_with_uname,0},
       {observer_wx,connect2,3},
       {os,cmd,1},
       {os_sup,command,3},
       {system_information,get_dynamic_libraries,0},
       {test_server_node,find_release,2},
       {yecc,os_process_size,0}]},
     {80,"OS process creation may require input validation",
      [{cpu_sup,start_portprogram,0},
       {erlang,open_port,2},
       {megaco_flex_scanner,open_drv_port,0},
       {memsup,start_portprogram,0},
       {os_mon,open_port,2},
     ...
     {15,"Keep OpenSSL updated for crypto module use (run with \"-V crypto\")",
      ['OTP-PUB-KEY','PKCS-FRAME',dtls,dtls_connection,dtls_connection_sup,
       ...
       {'CosFileTransfer_FileTransferSession_impl',receive_file,5},
       {'CosFileTransfer_FileTransferSession_impl',setup_connection_helper,4},
       {'CosFileTransfer_FileTransferSession_impl',setup_local,1},
       {'CosFileTransfer_FileTransferSession_impl',terminate,2},
       {asn1ct,compile_pass,1},
       {beam_lib,decrypt_abst_1,2},
       {c,c,2},
       {c,lc,1},
       {c,lc_batch,1},
       {c,nc,2},
       {client_server,init_connect,1},
       {client_server,start,0},
       {compile,encrypt,2},
       {compile,file,2},
       {compile,forms,2},
       ...
       {ct_config,decrypt_config_file,3},
       {ct_config,encrypt_config_file,3},
       {ct_config_plain,read_config,1},
       {ct_make,do_recompile,4},
       {ct_netconfc,handle_msg,2},
       {ct_netconfc,ssh_close,1},
       {ct_netconfc,ssh_open,1},
       {ct_netconfc,ssh_send,2},
       {ct_slave,spawn_remote_node,3},
       {ct_slave,ssh_setenv,3},
       {ct_snmp,get_next_values,3},
       {ct_snmp,get_values,3},
       {ct_snmp,load_mibs,1},
       {ct_snmp,override_community,2},
       {ct_snmp,override_contexts,2},
       {ct_snmp,override_notify,2},
       {ct_snmp,override_sysinfo,2},
       {ct_snmp,override_target_address,2},
       {ct_snmp,override_target_params,2},
       {ct_snmp,override_usm,2},
       {ct_snmp,override_vacm,2},
       {ct_snmp,set_values,4},
       {ct_snmp,setup_agent,7},
       {ct_snmp,setup_managed_agents,2},
       {ct_snmp,setup_manager,5},
       {ct_snmp,setup_users,1},
       {ct_snmp,setup_usm_users,2},
       {ct_snmp,takedown_managed_agents,2},
       {ct_snmp,takedown_users,1},
       {ct_snmp,takedown_usm_users,2},
       {ct_snmp,unload_mibs,1},
       {ct_ssh,do_recv_response,5},
       {ct_ssh,handle_msg,2},
       {ct_ssh,init,3},
       {ct_ssh,terminate,2},
       {dialyzer_analysis_callgraph,compile_common,7},
       {dialyzer_analysis_callgraph,compile_src,7},
       {dialyzer_cl,hc_cache,1},
       {dialyzer_utils,get_abstract_code_from_src,2},
       {dialyzer_utils,get_core_from_abstract_code,2},
       {diameter_tcp,accept,2},
       {diameter_tcp,send,3},
       {diameter_tcp,setopts,3},
       {diameter_tcp,ssl_info,1},
       {diameter_tcp,tls,3},
       {eldap,do_connect,3},
       {eldap,do_recv,3},
       {eldap,do_send,3},
       {eldap,do_start_tls,4},
       {eldap,do_unbind,1},
       {eldap,init,3},
       {eldap,ldap_closed_p,2},
       {eldap,loop,2},
       {escript,check_source,2},
       {escript,compile_source,5},
       {escript,parse_and_run,3},
       {ftp,accept_data_connection,1},
       {ftp,activate_connection,1},
       {ftp,close_connection,1},
       {ftp,handle_ctrl_result,2},
       {ftp,maybe_tls_upgrade,2},
       {ftp,peername,1},
       {ftp,send_message,2},
       {ftp,sockname,1},
       {hdlt_client,client,1},
       {hdlt_ctrl,client_clean_up,2},
       {hdlt_ctrl,create_client_content,4},
       {hdlt_ctrl,create_server_content,4},
       {hdlt_ctrl,del_dirs,2},
       {hdlt_ctrl,do_init,1},
       {hdlt_ctrl,ensure_remote_dir_exist,2},
       {hdlt_ctrl,ensure_remote_dir_exist,3},
       {hdlt_ctrl,maybe_create_remote_dir,2},
       {hdlt_ctrl,prepare_client_host,4},
       {hdlt_ctrl,prepare_server_host,1},
       {hdlt_ctrl,server_clean_up,3},
       {hdlt_server,maybe_start_crypto_and_ssl,1},
       {hdlt_slave,slave_running,5},
       {hdlt_slave,ssh_slave_start,2},
       {http_transport,accept,3},
       {http_transport,close,2},
       {http_transport,connect,4},
       {http_transport,controlling_process,3},
       {http_transport,getopts_ssl,2},
       {http_transport,listen_ssl,6},
       {http_transport,negotiate_ssl,2},
       {http_transport,peername,2},
       {http_transport,send,3},
       {http_transport,setopts,3},
       {http_transport,sockname,2},
       {httpc_handler,activate_once,1},
       {httpc_handler,close_socket,1},
       {httpc_handler,connect,4},
       {httpc_handler,connect_and_send_first_request,3},
       {httpc_handler,handle_call,3},
       {httpc_handler,handle_keep_alive_queue,2},
       {httpc_handler,handler_info,1},
       {httpc_handler,send_raw,2},
       {httpc_handler,send_raw,4},
       {httpc_handler,terminate,2},
       {httpc_handler,tls_tunnel,4},
       {httpc_handler,tls_upgrade,1},
       {httpc_request,do_send_body,4},
       {httpc_request,do_send_body,7},
       {httpc_request,send,3},
       {httpd_acceptor,acceptor_loop,8},
       {httpd_acceptor,handle_connection,7},
       {httpd_request_handler,continue_init,5},
       {httpd_request_handler,do_terminate,1},
       {httpd_request_handler,handle_body,3},
       {httpd_request_handler,handle_expect,2},
       {httpd_request_handler,handle_http_msg,2},
       {httpd_request_handler,handle_info,2},
       {httpd_request_handler,handle_next_request,2},
       {httpd_request_handler,reject_connection,2},
       {httpd_request_handler,terminate,2},
       {httpd_response,send_body,3},
       {httpd_response,send_chunk,3},
       {httpd_response,send_final_chunk,3},
       {httpd_response,send_header,3},
       {httpd_response,send_response_old,3},
       {httpd_response,send_status,3},
       {httpd_script_env,which_peercert,1},
       {httpd_socket,close,2},
       ...
       {megaco,try_load,2},
       {merl,compile,2},
       {mod_cgi,deliver_webpage,2},
       {mod_cgi,handle_body,6},
       {mod_cgi,send_headers,3},
       {mod_esi,deliver_webpage_chunk,3},
       {mod_esi,erl_scheme_webpage_whole,5},
       {mod_esi,handle_body,6},
       {mod_esi,send_headers,3},
       {mod_get,send,4},
       {mod_get,send_body,3},
       {mod_range,send_body,3},
       {mod_range,send_multiranges,4},
       {mod_range,send_part,4},
       {mod_range,send_part_start,6},
       {mod_trace,generate_trace_response,1},
       {orber_iiop_inproxy,decrease_counter,1},
       {orber_iiop_inproxy,handle_cast,2},
       {orber_iiop_inproxy,increase_counter,1},
       {orber_iiop_inproxy,init,1},
       {orber_iiop_inproxy,send_msg_error,5},
       {orber_iiop_inrequest,fragment_collector,8},
       {orber_iiop_inrequest,handle_message,5},
       {orber_iiop_inrequest,handle_request,8},
       {orber_iiop_inrequest,send_reply,3},
       {orber_iiop_net,access_allowed,3},
       {orber_iiop_net,get_sockethost,2},
       {orber_iiop_net,handle_call,3},
       {orber_iiop_net,parse_options,2},
       {orber_iiop_net_accept,net_accept,5},
       {orber_iiop_outproxy,handle_cast,2},
       {orber_iiop_outproxy,handle_reply,2},
       {orber_iiop_outproxy,init,1},
       {orber_socket,accept,3},
       {orber_socket,check_port,3},
       {orber_socket,close,2},
       {orber_socket,connect,5},
       {orber_socket,controlling_process,3},
       {orber_socket,listen,4},
       {orber_socket,multi_connect,7},
       {orber_socket,peercert,2},
       {orber_socket,peerdata,2},
       {orber_socket,peername,2},
       {orber_socket,post_accept,3},
       {orber_socket,setopts,3},
       {orber_socket,shutdown,3},
       {orber_socket,sockdata,2},
       {orber_socket,sockname,2},
       {orber_socket,write,3},
       {os_mon_mib,disk_table,1},
       {os_mon_mib,disk_table,3},
       {os_mon_mib,load,1},
       {os_mon_mib,load_table,1},
       {os_mon_mib,load_table,3},
       {os_mon_mib,node_update_disk_table,1},
       {os_mon_mib,unload,1},
       {os_mon_mib,update_disk_table,0},
       {otp_mib,appl_table,1},
       {otp_mib,appl_table,3},
       {otp_mib,erl_node_table,1},
       {otp_mib,erl_node_table,3},
       {otp_mib,load,1},
       {otp_mib,unload,1},
       {qlc_pt,compile_forms,2},
       {shell_default,c,2},
       {shell_default,lc,1},
       {snmp_ex2_manager,handle_call,3},
       {snmp_ex2_manager,handle_cast,2},
       {snmp_ex2_manager,register_user,0},
       {snmp_ex2_manager,start_manager,1},
       {snmp_ex2_manager,write_config,2},
       {typer,collect_one_file_info,2},
       {typer,extract,2}]},
     {10,"Dynamic creation of atoms can exhaust atom memory",
      [xmerl,xmerl_b64Bin,xmerl_b64Bin_scan,xmerl_eventp,xmerl_html,xmerl_lib,
       ...
       xmerl_xsd_type,
       {asn1ct,read_config_file,1},
       {asn1ct,read_config_file1,2},
       {beam_lib,f_p_s,2},
       {c,f_p_e,2},
       {compile,beam_consult_asm,1},
       {ct_config,do_start,1},
       {ct_config_plain,read_config,1},
       {ct_config_xml,do_read_xml_config,1},
       {ct_cover,get_spec_test,1},
       {ct_logs,open_ctlog,1},
       {ct_logs,read_summary,2},
       {ct_make,get_opts_from_emakefile,3},
       {ct_make,read_emakefile,2},
       {ct_netconfc,handle_data,2},
       {ct_netconfc,to_xml_doc,1},
       {ct_release_test,get_appup,2},
       {ct_release_test,get_runtime_deps,4},
       {ct_run,start_trace,1},
       {ct_snmp,override_community,2},
       {ct_snmp,override_contexts,2},
       {ct_snmp,override_notify,2},
       {ct_snmp,override_sysinfo,2},
       ...
       {ct_snmp,override_vacm,2},
       {ct_snmp,set_info,1},
       {ct_testspec,create_spec_tree,4},
       {ct_util,get_profile_data,3},
       {ct_util,read_opts,0},
       {ct_webtool,get_tools3,1},
       {dbg,rtp,1},
       {diameter_dbg,modules,0},
       {docgen_edoc_xml_cb,module,2},
       {docgen_edoc_xml_cb,overview,2},
       {docgen_otp_specs,module,2},
       {docgen_xmerl_xml_cb,'#element#',5},
       {docgen_xmerl_xml_cb,'#text#',1},
       {edoc_data,module,4},
       {edoc_data,overview,4},
       {edoc_data,type,2},
       {edoc_doclet,application_frame,4},
       {edoc_doclet,index_file,2},
       {edoc_doclet,modules_frame,4},
       {edoc_doclet,overview,4},
       {edoc_extract,source1,5},
       {edoc_layout,module,2},
       {edoc_layout,overview,2},
       {edoc_layout,type,2},
       {edoc_macros,type_macro,3},
       {edoc_wiki,parse_xml_1,2},
       {egd_font,load,1},
       {erlang,binary_to_atom,2},
       {erlang,binary_to_term,1},
       {erlang,list_to_atom,1},
       {eunit_data,get_file_tests,1},
       {eunit_lib,consult_file,1},
       {file,consult,1},
       {file,eval,1},
       {file,eval,2},
       {file,path_consult,2},
       ...
       {file,script,2},
       {hdlt,start,0},
       {httpd_sup,httpd_config,1},
       {ic,gen,2},
       {ic_genobj,new,1},
       {ic_options,read_cfg,2},
       {inet_db,add_rc,1},
       {inets,print_version_info,0},
       {inets,versions,0},
       {instrument,read_memory_data,1},
       {instrument,read_memory_status,1},
       {make,get_opts_from_emakefile,3},
       {make,read_emakefile,2},
       {megaco,ms1,0},
       {megaco_codec_transform,load_messages,2},
       {megaco_erl_dist_encoder,decode_message,3},
       {megaco_erl_dist_encoder,dmt,4},
       {megaco_erl_dist_encoder,dmt2,3},
       {msacc,from_file,1},
       {net_adm,host_file,0},
       {net_adm,world,1},
       {observer_trace_wx,read_settings,2},
       {otpsgml_layout,module,2},
       {otpsgml_layout,overview,2},
       {otpsgml_layout,type,2},
       {percept_image,load_font,0},
       {pool,start,2},
       {release_handler,consult,2},
       {release_handler,find_script,4},
       {release_handler,read_appspec,2},
       {reltool_server,read_config,2},
       {sudoku_gui,dialog,2},
       {system_information,build_app_table,2},
       {system_information,from_file,1},
       {system_information,get_application_from_path,1},
       {systools_make,check_relup,1},
       {systools_make,check_sys_config,1},
       {target_system,create,2},
       {test_server_ctrl,do_spec,2},
       {test_server_ctrl,parse_cmd_line,7},
       {test_server_ctrl,read_cover_file,1},
       {test_server_node,trc,1},
       {test_server_sup,is_app,1},
       {test_server_sup,is_appup,2},
       {test_server_sup,try_call_trace,1},
       {xref_parser,re,1}]}]
